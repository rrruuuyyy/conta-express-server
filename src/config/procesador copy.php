                        ...
                        if($xmls[$i]->estado === "nuevo" and $xmls[$i]->deducible === "si"){
                            ...
                            if($xmls[$i]->TipoDeComprobante === 'I'){
                                ...
                                try{
                                    ...
                                    $uuid_arre = strtoupper($xmls[$i]->UUID);
                                    $stmt->bindParam(':uuid', $uuid_arre);
                                ...
                                if( count($relaciones) === 1 ){
                                    ...
                                    for ($j=0; $j < count($relaciones); $j++) {
                                        ...
                                        try{
                                            ...
                                            if($compra){
                                                $sql_compra_int = "SELECT Serie, Folio FROM `docto-xml` WHERE UUID = {$compra->uuid}";
                                                try{
                                                    // Instanciar la base de datos
                                                    $db3 = new db();
                                                    // Conexión
                                                    $db3 = $db3->connect();
                                                    $ejecutar3 = $db->query($sql_docto);
                                                    $compra->xml = $ejecuta3->fetch(PDO::FETCH_OBJ);
                                                    $db3 = null;                                           
                                                } catch(PDOException $e){
                                                    $mensaje = array(
                                                        'status' => false,
                                    ...
                                    for ($j=0; $j < count($compras_encontradas); $j++) {
                                        for ($o=0; $o < count($conceptos); $o++) { 
                                            ...
                                            for ($l=0; $l < count($array_conceptos) ; $l++) {
                                                ...
                                                $encontrada = false;
                                                if( $array_conceptos[$l] === "{$Serie}-{$Folio}" || $array_conceptos[$l] === "{$Serie}{$Folio}" || $array_conceptos[$l] === "{$Serie}.{$Folio}" ){
                                                ...
                                                if( $encontrada === true ){
                                                    ...
                                                    try{
                                                        ...
                                                        $credito_dev->UUID_tipo_relacion = $xmls[$i]->UUID_tipo_relacion;
                                                        $credito_dev->Total = $descuento;
                                                        $credito_dev->Concepto = $conceptos[$o]->Importe;
                                                        $credito_dev->uuid = $xmls[$i]->UUID;
                                                        $credito_dev->especificado = false;
                                                        array_push( $devoluciones, $credito_dev );
                                                        $devoluciones = json_encode($devoluciones);
